"""
TDD tests for pinning chalk-action SHA in release-live-docker.yml.
...
WORKFLOW_FILE = Path(__file__).parents[3] / ".github" / "workflows" / "release-live-docker.yml"
...
def test_inline_comment_present():
    ...
    content = read_workflow()
    # Find the line with the chalk-action uses directive
    for line in content.splitlines():
        if "crashappsec/setup-chalk-action@" in line:
            # Comment must be on the same line and mention a date (YYYY-MM-DD)
            assert "#" in line, (
...
def test_grep_returns_no_output():
    """grep for setup-chalk-action@main must return exit code 1 (no matches)."""
